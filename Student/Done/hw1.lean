...
#eval implies true false  -- Should be false
#eval implies true true   -- Should be true
#eval implies false true  -- Should be true
#eval implies false false -- Should be true
...
#eval glue_funs double square 5   -- Should be 50 
#eval glue_funs square double 5   -- Should be 100


/-
This is because square is run before double, so you square first,
so you square, then double for the first and double, then square
for the second. They are different (and they should be) because
of PEMDAS (order of operations). 
The first is equivelant to (5^2) * 2;
The second is equivelant to (5 * 2)^2.
