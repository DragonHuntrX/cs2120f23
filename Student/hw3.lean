...
| Monday | Tuesday | Wednesday | Thursday | Friday => Work
| Saturday | Sunday => Play
...
#reduce day2kind Sunday     /- Should  be play -/
#reduce kind2reward Play    /- Should be health -/
#reduce day2reward Sunday   /- Should be health -/

#reduce day2kind Monday     /- Should be work -/
#reduce kind2reward Work    /- Should be Money -/
#reduce day2reward Monday   /- Should be Money -/
...
Answer here: 
Right associative, as adding parenthases to the types on the left 
does not change the overall type of the function, while placing 
the parenthesis on the left does change its type.
...
-- Here:
def triuple := triple 5 true "False"

#eval first triuple   -- Should be 5
#eval second triuple  -- Should be true
#eval third triuple   -- Should be "False"
...
def bitriuple : Nat → String → Bool → (Nat × String) × Bool
| a, b, c => ((a, b), c)

def bit := bitriuple 7 "True" false
